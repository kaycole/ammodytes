...
sl = mutate(sl, diet = NA, 
            ...
            common = sciname,
            common = replace(common, common %in% "Acipenser oxyrhynchus oxyrhynchus","Atlantic sturgeon"),  
            common = replace(common, common %in% "Centropristis striata","Black sea bass"),             
            common = replace(common, common %in% "Clupea harengus","Atlantic herring"),                    
            common = replace(common, common %in% "Cynoscion regalis","Weakfish"),                 
            common = replace(common, common %in% "Gadus morhua","Atlantic cod"),                       
            common = replace(common, common %in% "Illex illecebrosus","Northern shortfin squid"),                
            common = replace(common, common %in% "Leucoraja erinacea","Little skate"),                 
            common = replace(common, common %in% "Leucoraja ocellata","Winter skate"),                
            common = replace(common, common %in% "Lophiidae","Goosefish"),                          
            common = replace(common, common %in% "Melanogrammus aeglefinus","Haddock"),          
            common = replace(common, common %in% "Merluccius albidus","Offshore hake"),                
            common = replace(common, common %in% "Merluccius bilinearis","Silver hake"),             
            common = replace(common, common %in% "Mustelus canis","Dusky smooth-hound"),                     
            common = replace(common, common %in% "Paralichthys dentatus","Summer flounder"),             
            common = replace(common, common %in% "Pleuronectes ferruginea","Yellowtail flounder"),            
            common = replace(common, common %in% "Pleuronectes platessa","Eurpopean plaice"),             
            common = replace(common, common %in% "Pollachius","Pollock"),                         
            common = replace(common, common %in% "Pomatomus saltatrix","Bluefish"),               
            common = replace(common, common %in% "Reinhardtius hippoglossoides","Greenland halibut"),       
            common = replace(common, common %in% "Salmo salar","Atlantic salmon"),                       
            common = replace(common, common %in% "Salvelinus alpinus","Arctic char"),                 
            common = replace(common, common %in% "Scomber scombrus","Atlantic mackerel"),                  
            common = replace(common, common %in% "Stenotomus chrysops","Scup"),                
            common = replace(common, common %in% "Thunnus albacares","Yellowfin tuna"),                 
            common = replace(common, common %in% "Thunnus thynnus","Atlantic bluefin tuna"),                    
            common = replace(common, common %in% "Urophycis chuss","Red hake"),                   
            common = replace(common, common %in% "Urophycis tenuis","White hake"), 
            ...
            diet.metric.formal = replace(diet.metric.formal,diet.metric.formal %in% c("M","W"),"Mass"),
...
p = ggplot(data = filter(mean.sl.by.paper, !diet.metric.formal %in% c("undefined",NA)), 
       aes(y = mean.diet, x = reorder(comname, mean.diet, na.rm=TRUE), 
